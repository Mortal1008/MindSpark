...
def change_embedding_model():
    st.session_state["current_llm_settings"]["embedding_model"] = st.session_state[
...
with embedding_settings:
    ...
    embedding_model = st.selectbox(
        ...
        index=embedding_model_list.index(
            st.session_state["current_llm_settings"]["embedding_model"]
        ),
        disabled=disabled,
    ...
    if disabled:
        st.info(
            "You cannot change embedding model once you add documents in the knowledge base."
        )
    st.caption(
        "MindSpark supports most reranking models from `Hugging Face`. You may specify the models you want to use in the `config.py` file."
    )
    st.caption(
