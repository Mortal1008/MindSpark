...
def run():
    st.title("Setting_Advanced")
    st.write("这里是Setting_Advanced页面")


def change_top_k():
    st.session_state["current_llm_settings"]["top_k"] = st.session_state["top_k"]
    CONFIG_STORE.put(
        key="current_llm_settings", val=st.session_state["current_llm_settings"]
    )
    create_llm_instance()


def change_temperature():
    st.session_state["current_llm_settings"]["temperature"] = st.session_state[
        "temperature"
    ]
    CONFIG_STORE.put(
        key="current_llm_settings", val=st.session_state["current_llm_settings"]
...
def change_system_prompt():
    st.session_state["current_llm_settings"]["system_prompt"] = st.session_state[
        "system_prompt"
    ]
    CONFIG_STORE.put(
        key="current_llm_settings", val=st.session_state["current_llm_settings"]
...
def change_response_mode():
    st.session_state["current_llm_settings"]["response_mode"] = st.session_state[
        "response_mode"
    ]
    CONFIG_STORE.put(
        key="current_llm_settings", val=st.session_state["current_llm_settings"]
...
with advanced_settings:
    ...
    st.selectbox(
        ...
        key="response_mode",
        index=RESPONSE_MODE.index(
            st.session_state["current_llm_settings"]["response_mode"]
        ),  # simple_summarize by default
