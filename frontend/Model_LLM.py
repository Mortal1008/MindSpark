...
st.caption(
    "Support local models from Ollama and OpenAI compatible LLM APIs.",
...
def save_current_llm_info():
    ...
    if sp == "Ollama":
        ...
        if (
            st.session_state[model_key] is not None
            and st.session_state[api_key] is not None
            and st.session_state[base_key] is not None
        ):
            CONFIG_STORE.put(
                key="current_llm_info",
                val={
...
def update_llm_service_provider():
    ...
    CONFIG_STORE.put(
        key="llm_service_provider_selected",
        val={"llm_service_provider_selected": selected_option},
    )
...
def change_llm_api_key():
    ...
    is_valid = check_openai_llm(
        ...
        st.session_state.llm_api_endpoint,
        st.session_state.llm_api_key,
    )
    st.session_state[name + "_valid"] = is_valid
    CONFIG_STORE.put(
        key=name + "_valid",
        val={  # e.g. "OpenAI_api_key_valid"
...
# 重新加载和验证API key的函数
def reload_llm_api_key():
    name = option + "_api_key"  # e.g. "OpenAI_api_key"
    if name in st.session_state and st.session_state[name] is not None:
        # 使用已有的API key重新验证
        print("Reloading API key...")
        is_valid = check_openai_llm(
            st.session_state[option + "_model_selected"],
            st.session_state[option + "_api_base"],
        ...
        save_current_llm_info()
        create_llm_instance()  # 重新创建LLM实例
        if is_valid:
            st.success("API key has been successfully reloaded and validated")
            print("API key reloaded and validated successfully")
        else:
            st.error("API key validation failed after reloading")
            print("API key validation failed after reloading")
    else:
        st.warning("No API key found to reload")
        print("No API key found to reload")
        ...
        if option == "Ollama":
            ...
            if ollama.is_alive():
                ...
                st.selectbox(
                    ...
                    index=st.session_state.ollama_models.index(
                        st.session_state.ollama_model_selected
                    ),
            ...
            st.button(
                "🔄 Reload API key",
                on_click=reload_llm_api_key,
                help="重新验证和加载当前的API key，当key失效时使用",
            )
            st.selectbox(
                "Choose LLM API",
                llm["models"],
...
st.caption(
    "MindSpark supports `OpenAI` and all compatible LLM API like `DeepSeek`, `Moonshot` or `Zhipu`. You may specify the LLMs you want to use in the `config.py` file."
)
st.caption(
    "It is recommended to use `Ollama` if you need run the system without an Internet connection. Plase refer to the Ollama docs to download and use Ollama models."
)
